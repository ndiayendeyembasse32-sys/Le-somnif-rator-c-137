...
pygame.mixer.init()
son_sanction = None  # FIX : on initialise à None pour éviter un crash si le fichier est absent
try:
...
def actualiser_interface(statut):
    ...
    if "23H_START" in statut:
        ...
        if not son_actif:
            if son_sanction:  # FIX : on ne joue le son que s'il a bien été chargé
